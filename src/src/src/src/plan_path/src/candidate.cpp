...
typedef std::vector<State> CandidateSol;
...
std::vector<geometry_msgs::PolygonStamped> candidatePath;
...
void rviz_show_car(std::vector<std::vector<State>> &solutions, std::vector<std::vector<State>> &initial_solutions){ 
    ...
    for(int i = 0; i <candidatesolutions.size(); i++){
        ...
        for(int j= 0; j < candidatesolutions[i].size(); j++){
            candidatePath.push_back(myPolygon);
            for (int k = 0; k < candidatesolutions[i][j].size(); k += 3){         
                auto sol = candidatesolutions[i][j];   
                geometry_msgs::Point32* points = calpoints(sol[k]);
                candidatePath[num].polygon.points.push_back(points[3]);
                for (int i = 0; i < 4; i++){
                    // std::cout << points[i].x <<std::endl;
                    candidatePath[num].polygon.points.push_back(points[i]);
                     
                }
            }
            for(int k = candidatesolutions[i][j].size()-1; k >=0; k -= 3){  
                geometry_msgs::Point32* points = calpoints(candidatesolutions[i][j][k]);
                candidatePath[num].polygon.points.push_back(points[3]);
            }
            num++;
        }
    ...
    for (int i = 0; i < agv_count;i++){
        ...
        // else
         if(avoid_starts.find(i) != avoid_starts.end()&& i==0){
        ...
        if(avoid_starts.find(i) != avoid_starts.end() && i==0){
            ...
            std::cout<<i <<"    "<<solutions[i][avoid_starts[i].time+10]<<std::endl;
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < candidatesolutions.size();i++) {
        ...
        for(int j = 0; j < candidatesolutions[i].size();j++) {
            string str3 = str2;
            candidate_pub = nh.advertise<geometry_msgs::PolygonStamped>(str3.append(std::to_string(j)), 2);
