...
namespace dy_obs
{
  ...
  void UniformBspline::setPhysicalLimits(const double &vel, const double &acc, const double &tolerance, const double &vec_yaw)
  {
    limit_vel_ = vel;
    limit_acc_ = acc;
    limit_ratio_ = 1.1;
    feasibility_tolerance_ = tolerance;
    limit_vel_yaw= vec_yaw;
  ...
  bool UniformBspline::checkFeasibility(double &ratio, bool show)
  {
    ...
    // int dimension = control_points_.rows();//修改
    int dimension = control_points_.rows()-1;//修改


    /* check vel feasibility and insert points */
    double max_vel = -1.0;
    double max_vel_yaw = 0.1;

    double enlarged_vel_lim = limit_vel_ * (1.0 + feasibility_tolerance_) + 1e-4;
    // std::ofstream ofs;
    // std::string str11 = "/home/rancho/1hmy/ego-planner-swarm/record/checkFeasibility";
    // str11=str11+".txt";
    // ofs.open(str11,std::ios::app);
    //     ofs<<"enlarged_vel_lim is " <<enlarged_vel_lim<<endl;

    for (int i = 0; i < P.cols() - 1; ++i)
    {
      Eigen::VectorXd vel = p_ * (P.col(i + 1).head(2) - P.col(i).head(2)) / (u_(i + p_ + 1)- u_(i + 1));
      double angle = atan2(P.col(i + 1)(1)-P.col(i)(1),P.col(i + 1)(0)-P.col(i)(0));//输出角度在[-PI, PI]
      // if (fabs(vel(0)) > enlarged_vel_lim || fabs(vel(1)) > enlarged_vel_lim ||
          // fabs(vel(2)) > enlarged_vel_lim)//修改
      if (sqrt(vel(0)*vel(0)+vel(1)*vel(1))  > enlarged_vel_lim)//修改
      // if (fabs(vel(0)) > enlarged_vel_lim || fabs(vel(1)) > enlarged_vel_lim)//修改
      {
        if (show)
          {cout << "[Check]: Infeasible vel " << i << " :" << vel.transpose() << endl;
          cout << "[Check]: Infeasible vel_yaw " << i << " :" << angle << endl;}
        fea = false;

        // for (int j = 0; j < dimension; ++j)
        // {
          // max_vel = max(max_vel, fabs(vel(j)));
          max_vel = max(max_vel,sqrt(vel(0)*vel(0)+vel(1)*vel(1)));
        // }
        max_vel_yaw=max(max_vel_yaw,angle);
      }
    }
    // ofs<<"max_vel is " <<max_vel<<endl;

    /* acc feasibility */
    double max_acc = -1.0;
    double enlarged_acc_lim = limit_acc_ * (1.0 + feasibility_tolerance_) + 1e-4;
    // ofs<<"enlarged_acc_lim is " <<enlarged_acc_lim<<endl;
    ...
    for (int i = 0; i < P.cols() - 2; ++i)
    {
      ...
      Eigen::VectorXd acc = p_ * (p_ - 1) *
                            ...
                            (u_(i + p_ + 1) - u_(i + 2));//修改
      // Eigen::VectorXd acc = p_ * (p_ - 1) *
      //                       ((P.col(i + 2).head(2) - P.col(i + 1).head(2)) / (u_(i + p_ + 2) - u_(i + 2)) -
      //                        (P.col(i + 1).head(2) - P.col(i).head(2)) / (u_(i + p_ + 1) - u_(i + 1))) /
      //                       (u_(i + p_ + 1) - u_(i + 2));//修改
      // if (fabs(acc(0)) > enlarged_acc_lim || fabs(acc(1)) > enlarged_acc_lim ||
      //     fabs(acc(2)) > enlarged_acc_lim)//修改
      // if (fabs(acc(0)) > enlarged_acc_lim || fabs(acc(1)) > enlarged_acc_lim )//修改
      if (sqrt(acc(0)*acc(0)+acc(1)*acc(1)) > enlarged_acc_lim )//修改

      {
        ...
        // {
          ...
          max_acc = max(max_acc, sqrt(acc(0)*acc(0)+acc(1)*acc(1)));

        // }
      }
    }
    // ofs<<"max_acc is " <<max_acc<<endl;
    
    
    
    ratio = max(max_vel / limit_vel_, sqrt(fabs(max_acc) / limit_acc_));
    // ofs<<"ratio1 is " <<ratio<<endl;
    ratio = max(ratio,max_vel_yaw/limit_vel_yaw);
    // ofs<<"ratio2 is " <<ratio<<endl;
    // ofs.close();
    return fea;
  }
