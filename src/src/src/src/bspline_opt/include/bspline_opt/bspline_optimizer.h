...
// #include <plan_env/grid_map.h>//!!!!!!!!!!!!!!!!!!!!!!!!1
#include <plan_env/obj_predictor.h>//!!!!!!!!!!!!!!!!!!!!!!!!!!1
#include <ros/ros.h>
...
namespace YAML {
    template<>
    struct convert<tracpara> {
        static Node encode(const tracpara &rhs) {
            Node node;
            node.push_back(rhs.x);
            node.push_back(rhs.y);
            node.push_back(rhs.t);

            return node;
        }
        static bool decode(const Node &node, tracpara &rhs) {
            rhs.x = node["x"].as<double>();
            rhs.y = node["y"].as<double>();
            rhs.t = node["t"].as<double>();
...
namespace dy_obs
{
  ...
  public:
    ...
    void setEnvironment_( const fast_planner::ObjPredictor::Ptr mov_obj);
    ...
    void optimize();
    vector<Eigen::Vector3d> predict_dyobs;
    ControlPoints getControlPoints() { return cps_; };

    AStar::Ptr a_star_;
    std::vector<Eigen::Vector3d> ref_pts_;
    Eigen::Vector3d local_target_pt_; 
    Eigen::Vector3d occupoint;
    std::vector<Eigen::Vector3d> occupath;
    vector<vector<Eigen::Vector3d>> a_star_pathesa;
    ...
    bool BsplineOptimizeTrajRebound(Eigen::MatrixXd &optimal_points, double &final_cost,double ts,const ControlPoints &control_points,double t_current);
    
    bool BsplineOptimizeTrajRefine(const Eigen::MatrixXd &init_points, const double ts, Eigen::MatrixXd &optimal_points);
    void Setgloabaldata(GlobalTrajData global_data);
    inline int getOrder(void) { return order_; }
    inline double getSwarmClearance(void) { return swarm_clearance_; }
    void read_trac_yaml(std::string trac_yaml);
  ...
  private:
    ...
    GlobalTrajData the_global_data;
    ...
    double lambda5_;               // curve fitting
    double lambda7_,lambda6_;               // curve fitting
    // double lambda7_;               // curve fitting
...
#define INIT_min_ellip_dist_ 123456789.0123456789
    ...
    void calcbetwen_gloabl(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
    void calcbetwen_f_dyobs(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
    static int earlyExit(void *func_data, const double *x, const double *g, const double fx, const double xnorm, const double gnorm, const double step, int n, int k, int ls);
    void calcDistancestatic_CostRebound(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
    ...
    bool rebound_optimize_(double &final_cost,double current_tim);
