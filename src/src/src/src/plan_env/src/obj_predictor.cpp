...
void ObjPredictor::init() {
  ...
  // predict_dyobs_trajs_.reset(new vector<PolynomialPrediction>);
...
PolynomialPrediction ObjPredictor::get_dyobs_PredictionTraj() {
  return this->predict_dyobs_trajs_;
}
...
// void ObjPredictor::predictDyobs(list<Eigen::Vector4d> his) {
...
//     predict_dyobs_trajs_.setPolynomial(polys);
//     predict_dyobs_trajs_.setTime(t1, t2);
...
Eigen::Vector3d ObjPredictor::evaluateConstVel_dyobs( list<Eigen::Vector3d> his,double time)
{


  // std::vector<Eigen::Vector3d>::iterator list_it = his.end();
  // list<Eigen::Vector3d>::iterator list_it = his.begin();
  list<Eigen::Vector3d>::iterator list_it = his.end();
    ...
    q1 = (*list_it).head(2);
    t1 = (*list_it)(2);
    cout<<"list1 "<<*list_it<<endl;
    --list_it;
    q2 = (*list_it).head(2);
    t2 = (*list_it)(2);
    cout<<"list2"<<*list_it<<endl;

    Eigen::Matrix<double, 2, 2> p01, q12;
    q12.row(0) = q1.transpose();
    q12.row(1) = q2.transpose();
    cout<<"q12 "<<q12<<endl;

    Eigen::Matrix<double, 2, 2> At12;
    At12 << 1, t1, 1, t2;//t1t2不能相等
    ...
    vector<Eigen::Matrix<double, 4, 1>> polys(2);
    for (int j = 0; j < 2; ++j) {
      polys[j].setZero();
      polys[j].head(2) = p01.col(j);
    }
    ...
    // predict_dyobs_trajs_.setPolynomial_dyobs(polys);
    // predict_dyobs_trajs_.setTime(t1, t2);
    Eigen::Matrix<double, 2, 1> tv;
    tv << 1.0, pow(time-t2, 1);

    // cout << t-global_start_time_.toSec() << endl;

    Eigen::Vector3d pt;
    pt(0) = tv.dot(polys[0].head(2)), pt(1) = tv.dot(polys[1].head(2)), pt(2) = time;
    

    return pt;

    // return predict_dyobs_trajs_.evaluateConstVel_dyobs(time);
