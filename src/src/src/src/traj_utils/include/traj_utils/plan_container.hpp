...
namespace dy_obs
{
  ...
  public:
    ...
    vector<UniformBspline> local_traj_; // 局部均匀B样条轨迹，大小为3，分别对应位置、速度、加速度

    double global_duration_;                   // 全局轨迹的总时长
    ros::Time global_start_time_;              // 全局轨迹的开始时间
    double local_start_time_, local_end_time_; // 当前局部轨迹的开始和结束时间
    double time_increase_;                     // 最终轨迹相对于全局轨迹增加的时长
    double last_time_inc_;                     // 上一次局部轨迹相对于全局轨迹增加的时长
  ...
  struct PlanParameters
  {
    ...
    double max_vel_, max_acc_, max_jerk_,max_vel_yaw; // physical limits
